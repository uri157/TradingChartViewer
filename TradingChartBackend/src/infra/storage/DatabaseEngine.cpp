...
void DatabaseEngine::ObserverHandle::reset() {
    if (alive && engine) {
        engine->removeObserver(id);
    }
    engine = nullptr;
    id = 0;
    alive = false;
...
void DatabaseEngine::PriceLimitHandle::reset() {
    if (alive && engine) {
        engine->removePriceLimitObserver(id);
    }
    engine = nullptr;
    id = 0;
    alive = false;
...
void DatabaseEngine::rebuildCacheFromHistorical() {
...

    {
        ...
        priceObserversCopy.clear();
        priceObserversCopy.reserve(priceLimitObservers_.size());
        for (const auto& entry : priceLimitObservers_) {
            if (entry.observer) {
                priceObserversCopy.push_back(entry.observer);
            }
        }
        cache.clear();
        for (const auto& record : latest) {
...
DatabaseEngine::ObserverHandle DatabaseEngine::addObserver(core::ICacheObserver* observer, std::uint64_t mask) {
    if (!observer) {
        return {};
    }
    std::lock_guard<std::mutex> lock(cacheMutex);
    const std::size_t id = nextObserverId_++;
    cacheObservers_.push_back(CacheObserverEntry{id, observer, mask});
    return ObserverHandle(this, id);
}

void DatabaseEngine::removeObserver(std::size_t id) {
    std::lock_guard<std::mutex> lock(cacheMutex);
    for (std::size_t idx = 0; idx < cacheObservers_.size(); ++idx) {
        if (cacheObservers_[idx].id == id) {
            if (idx + 1 != cacheObservers_.size()) {
                cacheObservers_[idx] = cacheObservers_.back();
            }
            cacheObservers_.pop_back();
            return;
        }
    }
}
...
DatabaseEngine::PriceLimitHandle DatabaseEngine::addPriceLimitObserver(core::IPriceLimitObserver* observer) {
    if (!observer) {
        return {};
    }
    ...
    {
        std::lock_guard<std::mutex> lock(cacheMutex);
        id = nextPriceLimitObserverId_++;
        priceLimitObservers_.push_back(PriceLimitObserverEntry{id, observer});
    ...
    return PriceLimitHandle(this, id);
}

void DatabaseEngine::removePriceLimitObserver(std::size_t id) {
    std::lock_guard<std::mutex> lock(cacheMutex);
    for (std::size_t idx = 0; idx < priceLimitObservers_.size(); ++idx) {
        if (priceLimitObservers_[idx].id == id) {
            if (idx + 1 != priceLimitObservers_.size()) {
                priceLimitObservers_[idx] = priceLimitObservers_.back();
            }
            priceLimitObservers_.pop_back();
            return;
        }
    }
}
...
void DatabaseEngine::notifyObservers() {
    ...
    {
        std::lock_guard<std::mutex> lock(cacheMutex);
        for (const auto& entry : cacheObservers_) {
            const std::size_t index = static_cast<std::size_t>(entry.mask);
            if (index < cache.size()) {
                notifications.emplace_back(entry.observer, cache[index]);
            }
        }
    }
